...
Процедура ПроверитьРезультат_энргНомерПломбПриборовУчета_ВводПрибораУчета_Базовый(ОбъектыВПамяти, Результат)
	...
	|ИсходныйОбъект.Дата|ИсходныйОбъект.Организация |ИсходныйОбъект.Район	|ИсходныйОбъект.КлючПомещения	|ИсходныйОбъект.ТочкиУчета[0].ПриборУчета	|""""			|'2019-12-01 00:00:00'	|ИсходныйОбъект.Дата";
	ТаблицаЭталон = ТаблицаПоОписаниюGherkin(GherkinТаблица, ОбъектыВПамяти[0]);  	 	
	Ожидаем.Что(Результат.Количество()).Равно(ТаблицаЭталон.Количество());
	СравнениеТаблиц.ПроверитьРавенствоТаблиц(ТаблицаЭталон,Результат);
...
Процедура ПроверитьРезультат_энргПоказанияПриборовУчета_ВводПрибораУчета_Базовый(ОбъектыВПамяти, Результат)
	...
	|ИсходныйОбъект.Дата|ИсходныйОбъект.Организация |ИсходныйОбъект.Район	|ИсходныйОбъект.КлючПомещения	|ИсходныйОбъект.ТочкиУчета[0].ПриборУчета	|Справочники.энргШкалыПриборовУчета.ПолучитьСсылку(Новый УникальныйИдентификатор(""ae23461a-1fd0-11e9-80d4-00505693999d""))		|Справочники.энргТарифныеЗоны.ПолучитьСсылку(Новый УникальныйИдентификатор(""8f365e6b-f478-11e8-a9ca-902b34cfd89e""))	|0			|Число(128)	|Перечисления.энргСостояниеПоказаний.Приняты|Ложь	|Справочники.энргИсточникиПоказаний.ПустаяСсылка()	|Ложь		|'2019-12-01 00:00:00'	|ИсходныйОбъект.Дата|1				|ИсходныйОбъект.ТочкиУчета[0].ТочкаУчета";
	ТаблицаЭталон = ТаблицаПоОписаниюGherkin(GherkinТаблица, ОбъектыВПамяти[0]);  	 	
	Ожидаем.Что(Результат.Количество()).Равно(ТаблицаЭталон.Количество());
	СравнениеТаблиц.ПроверитьРавенствоТаблиц(ТаблицаЭталон,Результат);
КонецПроцедуры
...
Процедура ПроверитьРезультат_энргВидыДифференцированностиПриборовУчета_ЗаменаПрибораУчета_Базовый(ОбъектыВПамяти, Результат)
	...
	GherkinТаблица	= 
	"Период				|Организация				|Район					|КлючПомещения					|ПриборУчета								|Шкала																														|ВидДифференцированности								|ПериодРегистрации 	|ПериодНачисления		|ТочкаУчета
	|ИсходныйОбъект.Дата|ИсходныйОбъект.Организация |ИсходныйОбъект.Район	|ИсходныйОбъект.КлючПомещения	|ИсходныйОбъект.ТочкиУчета[0].ПриборУчета 	|Справочники.энргШкалыПриборовУчета.ПолучитьСсылку(Новый УникальныйИдентификатор(""ae23461a-1fd0-11e9-80d4-00505693999d""))	|Справочники.энргВидыДифференцированности.Однотарифный	|ИсходныйОбъект.Дата|'2019-12-01 00:00:00'	|ИсходныйОбъект.ТочкиУчета[0].ТочкаУчета";
...
Процедура ПроверитьРезультат_энргПоказанияПриборовУчета_ЗаменаПрибораУчета_Базовый(ОбъектыВПамяти, Результат)
	GherkinТаблица	= 
	"Период				|Организация				|Район					|КлючПомещения					|ПриборУчета									|Шкала																															|ТарифнаяЗона																											|Разделитель|Показание	|СостояниеПоказаний							|КС		|ИсточникПоказаний									|Переворот	|ПериодНачисления		|ПериодРегистрации	|НомерСтрокиТЧ	|ТочкаУчета
	|ИсходныйОбъект.Дата|ИсходныйОбъект.Организация |ИсходныйОбъект.Район	|ИсходныйОбъект.КлючПомещения	|ИсходныйОбъект.ТочкиУчета[0].ПриборУчета		|Справочники.энргШкалыПриборовУчета.ПолучитьСсылку(Новый УникальныйИдентификатор(""ae23461a-1fd0-11e9-80d4-00505693999d""))		|Справочники.энргТарифныеЗоны.ПолучитьСсылку(Новый УникальныйИдентификатор(""8f365e6b-f478-11e8-a9ca-902b34cfd89e""))	|0			|Число(0)	|Перечисления.энргСостояниеПоказаний.Приняты|Ложь	|Справочники.энргИсточникиПоказаний.ПустаяСсылка()	|Ложь		|'2019-12-01 00:00:00'	|ИсходныйОбъект.Дата|2				|ИсходныйОбъект.ТочкиУчета[0].ТочкаУчета
	|ИсходныйОбъект.Дата|ИсходныйОбъект.Организация |ИсходныйОбъект.Район	|ИсходныйОбъект.КлючПомещения	|ИсходныйОбъект.ТочкиУчета[0].ПриборУчетаВывод	|Справочники.энргШкалыПриборовУчета.ПолучитьСсылку(Новый УникальныйИдентификатор(""ae23461a-1fd0-11e9-80d4-00505693999d""))		|Справочники.энргТарифныеЗоны.ПолучитьСсылку(Новый УникальныйИдентификатор(""8f365e6b-f478-11e8-a9ca-902b34cfd89e""))	|0			|Число(15)	|Перечисления.энргСостояниеПоказаний.Приняты|Ложь	|Справочники.энргИсточникиПоказаний.ПустаяСсылка()	|Ложь		|'2019-12-01 00:00:00'	|ИсходныйОбъект.Дата|1				|ИсходныйОбъект.ТочкиУчета[0].ТочкаУчета";
	ТаблицаЭталон = ТаблицаПоОписаниюGherkin(GherkinТаблица, ОбъектыВПамяти[0]);  	 	
	Ожидаем.Что(Результат.Количество()).Равно(ТаблицаЭталон.Количество());
	СравнениеТаблиц.ПроверитьРавенствоТаблиц(ТаблицаЭталон,Результат);
КонецПроцедуры
...
Процедура ПроверитьРезультат_энргПоказанияПриборовУчета_ВыводПрибораУчета_Базовый(ОбъектыВПамяти, Результат)
	...
	GherkinТаблица	= 
	"Период				|Организация				|Район					|КлючПомещения					|ПриборУчета									|Шкала																															|ТарифнаяЗона																											|Разделитель|Показание	|СостояниеПоказаний							|КС		|ИсточникПоказаний									|Переворот	|ПериодНачисления		|ПериодРегистрации	|НомерСтрокиТЧ	|ТочкаУчета
	|ИсходныйОбъект.Дата|ИсходныйОбъект.Организация |ИсходныйОбъект.Район	|ИсходныйОбъект.КлючПомещения	|ИсходныйОбъект.ТочкиУчета[0].ПриборУчетаВывод	|Справочники.энргШкалыПриборовУчета.ПолучитьСсылку(Новый УникальныйИдентификатор(""ae23461a-1fd0-11e9-80d4-00505693999d""))		|Справочники.энргТарифныеЗоны.ПолучитьСсылку(Новый УникальныйИдентификатор(""8f365e6b-f478-11e8-a9ca-902b34cfd89e""))	|0			|Число(152)	|Перечисления.энргСостояниеПоказаний.Приняты|Ложь	|Справочники.энргИсточникиПоказаний.ПустаяСсылка()	|Ложь		|'2019-12-01 00:00:00'	|ИсходныйОбъект.Дата|1				|ИсходныйОбъект.ТочкиУчета[0].ТочкаУчета";
	ТаблицаЭталон = ТаблицаПоОписаниюGherkin(GherkinТаблица, ОбъектыВПамяти[0]);  	 	
	Ожидаем.Что(Результат.Количество()).Равно(ТаблицаЭталон.Количество());
	СравнениеТаблиц.ПроверитьРавенствоТаблиц(ТаблицаЭталон,Результат);
...
Процедура ПроверитьРезультат_энргСостояниеПриборовУчетаАбонента_ВыводПрибораУчета_Базовый(ОбъектыВПамяти, Результат)
	...
	GherkinТаблица	= 
	"Период				|Организация				|Район					|КлючПомещения					|ТочкаУчета								|ПриборУчета									|Состояние										|ПериодНачисления		|ПериодРегистрации	
	|ИсходныйОбъект.Дата|ИсходныйОбъект.Организация |ИсходныйОбъект.Район	|ИсходныйОбъект.КлючПомещения	|ИсходныйОбъект.ТочкиУчета[0].ТочкаУчета|ИсходныйОбъект.ТочкиУчета[0].ПриборУчетаВывод	|Перечисления.энргСостояниеПриборовУчета.Вывод	|'2019-12-01 00:00:00'	|ИсходныйОбъект.Дата";
...
Процедура ПроверитьНаличиеТаблиц(КлючиТаблицДвижений)
	...
	Ожидаем.Что(КлючиТаблицДвижений.Количество()).Равно(7);
